...
This module implements optimality metrics, approximation ratios, and candidate discard rates.
-/
...
def IsOptimalGenerator (S : Set U) : Prop :=
  IsGeneratingSet cl S ∧
    ∀ T, IsGeneratingSet cl T → S.toFinite.toFinset.card ≤ T.toFinite.toFinset.card

/-- The approximation ratio compares the sieve's output to the optimal size. -/
...
/-- Search Work represents the discard rate W. -/
...
theorem searchWork_limit (sieve_output : Set U) :
  ...
  let b_omega := sieve_output.toFinite.toFinset.card
  have h_eq : (fun (k : ℕ) => searchWork k sieve_output) =ᶠ[atTop]
      (fun (k : ℕ) => 1 - (b_omega : ℚ) * ((k : ℚ)⁻¹)) := by
