...
This module implements an adversarial closure space exhibiting worst-case greedy bloat.
-/
...
open Classical in
/-- Adversarial closure operator returning `Set.univ` if `e_star ∈ S`, and `S` otherwise. -/
noncomputable def advCl (S : Set U) : Set U :=
  if e_star ∈ S then Set.univ else S

open Classical in
instance : ClosureSystem.ClosureSystem (advCl e_star) where
  extensive X := by
    dsimp [advCl]
    split_ifs
...
theorem searchWork_adv_trap_limit :
  ...
  exact SearchMetrics.searchWork_limit {e_star}
