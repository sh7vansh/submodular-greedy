...
This module implements the submodular approximation bound and discrete greedy bounds
for adaptive generator extraction.
-/


open Real
open ClosureSystem SearchMetrics

namespace SubmodularGreedy
...
structure RankFunction (U : Type) [Fintype U] where
  /-- The underlying evaluation function from sets to real values. -/
  f : Set U → ℝ
  normalized : f ∅ = 0
...
open Classical in
lemma advCl_max_marginal_gain (cl : Set U → Set U) (e_star : U)
...
open Classical in
lemma advCl_opt_size (cl : Set U → Set U) (opt : Set U) (e_star : U)
    (h_cl : cl = fun S => if _h : e_star ∈ S then Set.univ else S)
...
open Classical in
/-- Natural-indexed sequence of generator sets extracted by a discovery operator. -/
@[nolint defsWithUnderscore]
noncomputable def B_nat (cl : Set U → Set U) (phi : DiscoveryOperator U) : ℕ → Set U
  | 0 => ∅
  | n + 1 =>
    if h : cl (B_nat cl phi n) ⊂ Set.univ then
      B_nat cl phi n ∪ {phi (cl (B_nat cl phi n)) h}
    else B_nat cl phi n
...
lemma B_seq_add_one (cl : Set U → Set U) (phi : DiscoveryOperator U) (o : Ordinal) :
    B_seq cl phi (o + 1) =
      if h : cl (B_seq cl phi o) ⊂ Set.univ then
        B_seq cl phi o ∪ {phi (cl (B_seq cl phi o)) h}
      else B_seq cl phi o := by
  dsimp [B_seq]
...
omit [Fintype U] in
lemma opt_empty_of_card_zero [Finite U] (opt : Set U)
    (h : opt.toFinite.toFinset.card = 0) : opt = ∅ := by
...
lemma closureRank_mono (cl : Set U → Set U) [ClosureSystem.ClosureSystem cl] :
...
omit [Fintype U] in
lemma cl_union_eq_cl_cl_union (cl : Set U → Set U) [ClosureSystem.ClosureSystem cl]
    (B : Set U) (x : U) :
    cl (B ∪ {x}) = cl (cl B ∪ {x}) := by
...
lemma closureRank_union_eq (cl : Set U → Set U) [ClosureSystem.ClosureSystem cl]
    (B : Set U) (x : U) :
...
lemma closureRank_cl_eq (cl : Set U → Set U) [ClosureSystem.ClosureSystem cl] (S : Set U) :
...
lemma submodular_finset_le (f : Set U → ℝ) (hf : IsSubmodular f)
  ...
  | @insert a s has ih =>
    ...
    have h_split : f (insert a (A ∪ ↑s)) - f A =
        (f (insert a (A ∪ ↑s)) - f (A ∪ ↑s)) + (f (A ∪ ↑s) - f A) := by ring
    rw [h_split]
...
lemma deficit_zero_le (cl : Set U → Set U) (opt : Set U) (h_submod : IsSubmodularClosure cl)
  ...
  have h_each_le : ∀ x ∈ opt.toFinite.toFinset,
      closureRank cl (∅ ∪ {x}) - closureRank cl ∅ ≤ Δ := by
    intro x hx
    rw [Set.empty_union]
    dsimp [closureRank]
    ...
    linarith
  have h_sum_le := sum_le_card_mul opt.toFinite.toFinset
      (fun x => closureRank cl (∅ ∪ {x}) - closureRank cl ∅) Δ h_each_le
  linarith
...
theorem analytic_greedy_bound_m (k m : ℕ) (hm : 1 ≤ m) (h_km : m < k) (Δ : ℝ) (hΔ : 0 < Δ)
  ...
  have h_sub_log : Real.log (u 0) - Real.log (u (k - m)) ≤ Real.log Δ := by
    ...
    have h_log_prod : Real.log ((m : ℝ) * Δ) = Real.log (m : ℝ) + Real.log Δ :=
      Real.log_mul hm_pos.ne' hΔ.ne'
...
/-- The remaining rank deficit at step `i` of the adaptive greedy procedure. -/
...
omit [Fintype U] in
lemma B_nat_card_le [Finite U] (cl : Set U → Set U) (phi : DiscoveryOperator U) (n : ℕ) :
  ...
  | succ n ih =>
    ...
    · have h_ins : (B_nat cl phi n ∪ {phi (cl (B_nat cl phi n)) h}).toFinite.toFinset =
        ...
        simp
      rw [h_ins]
      have h_card := Finset.card_insert_le
          (phi (cl (B_nat cl phi n)) h) (B_nat cl phi n).toFinite.toFinset
      omega
...
lemma adaptiveGreedy_gain_ge (cl : Set U → Set U) (C : Set U) (hC : C ⊂ Set.univ)
    (x : U) (hx : x ∈ Cᶜ) :
    (cl (C ∪ {x})).toFinite.toFinset.card ≤
      (cl (C ∪ {adaptiveGreedyPhi cl C hC})).toFinite.toFinset.card :=
  ClosureSystem.adaptiveGreedyPhi_max cl C hC x hx
...
omit [Fintype U] in
lemma B_nat_eventually_stabilizes [Finite U] (cl : Set U → Set U) (phi : DiscoveryOperator U) :
  ...
  have h_card : ∀ n, n ≤ (B_nat cl phi n).toFinite.toFinset.card := by
    ...
    | succ n ih =>
      ...
      have h_fin_ss : (B_nat cl phi n).toFinite.toFinset ⊂
          (B_nat cl phi (n + 1)).toFinite.toFinset := by
...
/-- The finite termination index where the greedy sequence stabilizes. -/
noncomputable def terminationIndex (cl : Set U → Set U) (phi : DiscoveryOperator U) : ℕ :=
  Nat.find (B_nat_eventually_stabilizes cl phi)

lemma B_nat_strict_mono (cl : Set U → Set U)
...
open Classical in
/-- Single step extension operator adding a newly discovered generator if available. -/
@[nolint defsWithUnderscore]
...
lemma B_nat_stabilizes_add (cl : Set U → Set U) (phi : DiscoveryOperator U) (k : ℕ) :
  ...
  | zero => rfl
  | succ k ih =>
    have h_step : B_nat cl phi (terminationIndex cl phi + k + 1) =
        B_nat cl phi (terminationIndex cl phi + k) := by
      change B_step cl phi (B_nat cl phi (terminationIndex cl phi + k)) =
        B_nat cl phi (terminationIndex cl phi + k)
      rw [ih]
...
lemma B_nat_stabilizes (cl : Set U → Set U)
...
lemma sieve_output_eq_B_nat_termination (cl : Set U → Set U)
...
lemma B_nat_card_eq_of_lt (cl : Set U → Set U)
    (phi : DiscoveryOperator U) (n : ℕ)
...
lemma greedy_card_eq_terminationIndex (cl : Set U → Set U) :
    (sieve_output cl (adaptiveGreedyPhi cl)).toFinite.toFinset.card =
      terminationIndex cl (adaptiveGreedyPhi cl) := by
  rw [sieve_output_eq_B_nat_termination cl (adaptiveGreedyPhi cl)]
  exact B_nat_card_eq_of_lt cl (adaptiveGreedyPhi cl)
      (terminationIndex cl (adaptiveGreedyPhi cl)) le_rfl

lemma closureRank_step_ge_one (cl : Set U → Set U) [ClosureSystem.ClosureSystem cl] (n : ℕ)
    (hn : n < terminationIndex cl (adaptiveGreedyPhi cl)) :
    1 ≤ closureRank cl (B_nat cl (adaptiveGreedyPhi cl) (n + 1)) -
      closureRank cl (B_nat cl (adaptiveGreedyPhi cl) n) := by
  ...
  · let x := adaptiveGreedyPhi cl (cl (B_nat cl (adaptiveGreedyPhi cl) n)) h_cl
    have hx_not : x ∉ cl (B_nat cl (adaptiveGreedyPhi cl) n) :=
      ClosureSystem.novelty_of_adaptiveGreedyPhi cl _ h_cl
    have hx_in_succ : x ∈ cl (B_nat cl (adaptiveGreedyPhi cl) (n + 1)) := by
      dsimp [B_nat]
      rw [dif_pos h_cl]
      apply ClosureSystem.extensive
      exact Set.subset_union_right (Set.mem_singleton x)
    have h_cl_sub : cl (B_nat cl (adaptiveGreedyPhi cl) n) ⊆
        cl (B_nat cl (adaptiveGreedyPhi cl) (n + 1)) :=
      ClosureSystem.monotone _ _ (B_nat_subset_succ cl (adaptiveGreedyPhi cl) n)
    have h_cl_ss : cl (B_nat cl (adaptiveGreedyPhi cl) n) ⊂
        cl (B_nat cl (adaptiveGreedyPhi cl) (n + 1)) := by
...
lemma closureRank_diff_ge (cl : Set U → Set U) [ClosureSystem.ClosureSystem cl] (j d : ℕ)
    (h : j + d ≤ terminationIndex cl (adaptiveGreedyPhi cl)) :
    (d : ℝ) ≤ closureRank cl (B_nat cl (adaptiveGreedyPhi cl) (j + d)) -
      closureRank cl (B_nat cl (adaptiveGreedyPhi cl) j) := by
  induction d with
  | zero =>
    simp
  | succ d ih =>
    ...
    have h_split : closureRank cl (B_nat cl (adaptiveGreedyPhi cl) (j + (d + 1))) -
          closureRank cl (B_nat cl (adaptiveGreedyPhi cl) j) =
        (closureRank cl (B_nat cl (adaptiveGreedyPhi cl) (j + d + 1)) -
          closureRank cl (B_nat cl (adaptiveGreedyPhi cl) (j + d))) +
        (closureRank cl (B_nat cl (adaptiveGreedyPhi cl) (j + d)) -
          closureRank cl (B_nat cl (adaptiveGreedyPhi cl) j)) := by
...
lemma B_nat_termination_cl_univ (cl : Set U → Set U) [ClosureSystem cl]
    (phi : DiscoveryOperator U) (h_nov : ∀ (C : Set U) (h : C ⊂ Set.univ), phi C h ∉ C) :
    cl (B_nat cl phi (terminationIndex cl phi)) = Set.univ := by
  have h_stab : B_nat cl phi (terminationIndex cl phi + 1) =
      B_nat cl phi (terminationIndex cl phi) :=
    Nat.find_spec (B_nat_eventually_stabilizes cl phi)
  dsimp [B_nat] at h_stab
  split_ifs at h_stab with h_ss
  · have h_in : phi (cl (B_nat cl phi (terminationIndex cl phi))) h_ss ∈
        B_nat cl phi (terminationIndex cl phi) := by
      have : phi (cl (B_nat cl phi (terminationIndex cl phi))) h_ss ∈
          B_nat cl phi (terminationIndex cl phi) ∪
          {phi (cl (B_nat cl phi (terminationIndex cl phi))) h_ss} :=
        Set.subset_union_right (Set.mem_singleton _)
      rwa [h_stab] at this
    have h_in_cl : phi (cl (B_nat cl phi (terminationIndex cl phi))) h_ss ∈
        cl (B_nat cl phi (terminationIndex cl phi)) :=
      ClosureSystem.extensive _ h_in
    have h_not := h_nov (cl (B_nat cl phi (terminationIndex cl phi))) h_ss
    exact (h_not h_in_cl).elim
  · apply Set.Subset.antisymm (Set.subset_univ _)
    intro x _
    by_contra hx
    have : cl (B_nat cl phi (terminationIndex cl phi)) ⊂ Set.univ := by
      apply Set.ssubset_iff_subset_ne.mpr ⟨Set.subset_univ _, ?_⟩
      intro h_univ
      rw [h_univ] at hx
      exact hx (Set.mem_univ x)
    exact h_ss this

lemma sieve_output_isGeneratingSet (cl : Set U → Set U) [ClosureSystem cl]
    (phi : DiscoveryOperator U) (h_nov : ∀ (C : Set U) (h : C ⊂ Set.univ), phi C h ∉ C) :
    IsGeneratingSet cl (sieve_output cl phi) := by
  dsimp [IsGeneratingSet]
  rw [sieve_output_eq_B_nat_termination cl phi]
  exact B_nat_termination_cl_univ cl phi h_nov

theorem adaptiveGreedy_isGeneratingSet (cl : Set U → Set U) [ClosureSystem cl] :
    IsGeneratingSet cl (sieve_output cl (adaptiveGreedyPhi cl)) :=
  sieve_output_isGeneratingSet cl (adaptiveGreedyPhi cl)
    (ClosureSystem.novelty_of_adaptiveGreedyPhi cl)

lemma greedyDeficit_mono
    (cl : Set U → Set U) [ClosureSystem.ClosureSystem cl] (opt greedy : Set U)
    (_h_submod : IsSubmodularClosure cl) (_h_opt : IsOptimalGenerator cl opt)
    (_h_is_greedy_output :
      greedy = ClosureSystem.sieve_output cl (ClosureSystem.adaptiveGreedyPhi cl))
    (i : ℕ) (_hi : i < greedy.toFinite.toFinset.card - opt.toFinite.toFinset.card) :
...
lemma greedyDeficit_pos
    (cl : Set U → Set U) [ClosureSystem.ClosureSystem cl] (opt greedy : Set U)
    (_h_submod : IsSubmodularClosure cl) (h_opt : IsOptimalGenerator cl opt)
    (h_is_greedy_output :
      greedy = ClosureSystem.sieve_output cl (ClosureSystem.adaptiveGreedyPhi cl))
  ...
  have hi_T : i + 1 < terminationIndex cl (adaptiveGreedyPhi cl) := by
    ...
    · have h_gz := greedy_empty_of_opt_zero cl opt greedy h_opt h_is_greedy_output h_opt_zero
      ...
      omega
    · have h_greedy : IsGeneratingSet cl greedy := by
        rw [h_is_greedy_output]
        exact adaptiveGreedy_isGeneratingSet cl
  ...
  dsimp [closureRank]
  have h_card_lt :
      (cl (B_nat cl (adaptiveGreedyPhi cl) (i + 1))).toFinite.toFinset.card < Fintype.card U := by
    obtain ⟨x, hx⟩ := Set.nonempty_compl.mpr h_not_univ.ne
    have h_fin_ne :
        (cl (B_nat cl (adaptiveGreedyPhi cl) (i + 1))).toFinite.toFinset ≠ Finset.univ := by
      intro h_univ
      ...
      exact hx hx_in
    have h_ss_fin :
        (cl (B_nat cl (adaptiveGreedyPhi cl) (i + 1))).toFinite.toFinset ⊂ Finset.univ :=
...
lemma greedyDeficit_ukm
    (cl : Set U → Set U) [ClosureSystem.ClosureSystem cl] (opt greedy : Set U)
    (_h_submod : IsSubmodularClosure cl) (h_opt : IsOptimalGenerator cl opt)
    (h_is_greedy_output :
      greedy = ClosureSystem.sieve_output cl (ClosureSystem.adaptiveGreedyPhi cl)) :
    (opt.toFinite.toFinset.card : ℝ) ≤
      greedyDeficit cl (greedy.toFinite.toFinset.card - opt.toFinite.toFinset.card) := by
...
lemma greedyDeficit_step_le
    (cl : Set U → Set U) [ClosureSystem.ClosureSystem cl] (opt greedy : Set U)
    (h_submod : IsSubmodularClosure cl) (h_opt : IsOptimalGenerator cl opt)
    (h_is_greedy_output :
      greedy = ClosureSystem.sieve_output cl (ClosureSystem.adaptiveGreedyPhi cl))
    (i : ℕ) (hi : i < greedy.toFinite.toFinset.card - opt.toFinite.toFinset.card) :
    1 / (opt.toFinite.toFinset.card : ℝ) ≤
      (greedyDeficit cl i - greedyDeficit cl (i + 1)) / greedyDeficit cl i := by
  have h_T : greedy.toFinite.toFinset.card = terminationIndex cl (adaptiveGreedyPhi cl) := by
    rw [h_is_greedy_output]
    exact greedy_card_eq_terminationIndex cl
  ...
  have h_Bi_succ : B_nat cl (adaptiveGreedyPhi cl) (i + 1) =
      B_nat cl (adaptiveGreedyPhi cl) i ∪ {x} := by
    dsimp [B_nat]
    rw [dif_pos hC]
  ...
  have h_rank_Cx : closureRank cl (C ∪ {x}) =
      closureRank cl (B_nat cl (adaptiveGreedyPhi cl) (i + 1)) := by
    rw [h_Bi_succ]
  ...
  have h_gain : greedyDeficit cl i - greedyDeficit cl (i + 1) =
      closureRank cl (C ∪ {x}) - closureRank cl C := by
    dsimp [greedyDeficit]
    rw [h_rank_Cx, h_rank_C]
    ring
  rw [← h_def_i, ← h_gain] at h_step_le
  have h_def_pos : 0 < greedyDeficit cl i := by
    have h_mono := greedyDeficit_mono cl opt greedy h_submod h_opt h_is_greedy_output i hi
    have h_pos := greedyDeficit_pos cl opt greedy h_submod h_opt h_is_greedy_output i hi
    linarith
...
theorem greedy_submodular_bound (cl : Set U → Set U)
    [ClosureSystem.ClosureSystem cl] (opt greedy : Set U)
  ...
  · have h_opt_pos : 1 ≤ opt.toFinite.toFinset.card := by omega
    ...
    · have h_gt : opt.toFinite.toFinset.card < greedy.toFinite.toFinset.card := by omega
      ...
      have h_Δ_pos : 0 < Δ := by
        ...
        nlinarith
      have h_bound := analytic_greedy_bound_m
          greedy.toFinite.toFinset.card opt.toFinite.toFinset.card
        (by exact_mod_cast h_opt_pos) (by exact_mod_cast h_gt) Δ h_Δ_pos (greedyDeficit cl)
        (by
          intro i hi
          exact greedyDeficit_step_le cl opt greedy h_submod h_opt h_is_greedy_output i hi
        )
        (by
          intro i hi
          exact greedyDeficit_pos cl opt greedy h_submod h_opt h_is_greedy_output i hi
        )
        (by
          intro i hi
          exact greedyDeficit_mono cl opt greedy h_submod h_opt h_is_greedy_output i hi
        )
        (by
          exact h_def_le
        )
        ...
        (by
          exact greedyDeficit_ukm cl opt greedy h_submod h_opt h_is_greedy_output
        )
      exact h_bound
...
lemma B_nat_step [LinearOrder U] [WellFoundedLT U] (cl : Set U → Set U) (e_star : U)
  ...
  · intro h_lt2 h_m
    have h_all : ∀ y ∈ Bkᶜ, y = e_star := by
      ...
      have h_m_def : m =
          WellFounded.min wellFounded_lt (cl Bk)ᶜ (Set.nonempty_compl.mpr h_cl_ss.ne) := rfl
...
lemma B_nat_card_and_not_mem [LinearOrder U] [WellFoundedLT U] (cl : Set U → Set U) (e_star : U)
  ...
  | succ k ih =>
    ...
    rcases B_nat_step cl e_star h_cl h_worst k ih_card ih_not_mem hk_lt with
      ⟨h_ss, h_def, hm_not, hm_ne⟩
    have h_card_succ : (B_nat cl fixedPriorityPhi (k + 1)).toFinite.toFinset.card = k + 1 := by
      rw [h_def]
      have hs : (B_nat cl fixedPriorityPhi k ∪
          {fixedPriorityPhi (cl (B_nat cl fixedPriorityPhi k)) h_ss}).toFinite.toFinset =
        insert (fixedPriorityPhi (cl (B_nat cl fixedPriorityPhi k)) h_ss)
          (B_nat cl fixedPriorityPhi k).toFinite.toFinset := by
        ext y
        simp
      rw [hs]
...
open Classical in
theorem advCl_greedy_size_worst_case [LinearOrder U] [WellFoundedLT U]
    (cl : Set U → Set U) (greedy : Set U) (e_star : U)
    (h_cl : cl = fun S => if _h : e_star ∈ S then Set.univ else S)
    (h_worst : ∀ x, x ≤ e_star) -- e_star is the LAST element evaluated
    (h_greedy : greedy = ClosureSystem.sieve_output cl ClosureSystem.fixedPriorityPhi) :
...
open Classical in
/-- A fixed-priority ordering violates the submodular logarithmic bound on the adversarial trap.
    Because the fixed priority order evaluates the optimal generator last, it selects
    all elements in U, exceeding (ln Δ + 1) * |B_opt| for |U| ≥ 3. -/
theorem fixed_priority_violates_submodular_bound [LinearOrder U] [WellFoundedLT U]
    (cl : Set U → Set U) (opt greedy : Set U) (e_star : U)
    (h_cl : cl = fun S => if _h : e_star ∈ S then Set.univ else S)
    (h_worst : ∀ x, x ≤ e_star)
    (h_is_greedy_output : greedy = ClosureSystem.sieve_output cl ClosureSystem.fixedPriorityPhi)
    (h_opt : IsOptimalGenerator cl opt)
    (h_n : 3 ≤ Fintype.card U) :
    ¬ ( (greedy.toFinite.toFinset.card : ℝ) ≤
        (Real.log (maxMarginalGain cl) + 1) * (opt.toFinite.toFinset.card : ℝ) ) := by
  ...
  have h_greedy_size : greedy.toFinite.toFinset.card = Fintype.card U :=
    advCl_greedy_size_worst_case cl greedy e_star h_cl h_worst h_is_greedy_output
  have h_greedy_real : (greedy.toFinite.toFinset.card : ℝ) = Fintype.card U :=
    by exact_mod_cast h_greedy_size
  rw [h_greedy_real]
...
/-- Average rank over all subsets for a complement-symmetric rank function. -/
theorem randomized_expected_bound [DecidableEq U] (cl : Set U → Set U) (n : ℝ)
    (_h_n : n = Fintype.card U)
    (h_symm : ∀ (S : Finset U), closureRank cl S + closureRank cl (Sᶜ : Finset U) = n) :
    expectedUniformRank cl = n / 2 := by
  have h_sum_compl : ∑ S : Finset U, closureRank cl (Sᶜ : Finset U) =
      ∑ S : Finset U, closureRank cl S := by
  ...
  have h_double_sum : 2 * ∑ S : Finset U, closureRank cl (S : Set U) = ∑ S : Finset U, n := by
    calc
      2 * ∑ S : Finset U, closureRank cl (S : Set U) =
        (∑ S : Finset U, closureRank cl (S : Set U)) +
          (∑ S : Finset U, closureRank cl (S : Set U)) := by ring
      _ = (∑ S : Finset U, closureRank cl (S : Set U)) +
          (∑ S : Finset U, closureRank cl (Sᶜ : Finset U)) := by rw [h_sum_compl]
      _ = ∑ S : Finset U, (closureRank cl (S : Set U) + closureRank cl (Sᶜ : Finset U)) :=
  ...
  have h_card_finset : (Fintype.card (Finset U) : ℝ) = (2 : ℝ) ^ Fintype.card U := by
    ...
    push_cast
    rfl
  have h_total_rank : ∑ S : Finset U, closureRank cl (S : Set U) =
      ((2 : ℝ) ^ Fintype.card U * n) / 2 := by
    calc
      ∑ S : Finset U, closureRank cl (S : Set U) =
        (2 * ∑ S : Finset U, closureRank cl (S : Set U)) / 2 := by ring
